...
quotes = [ 'Throw me to the wolves and I will return leading the pack',
           'Luck is what happens when preparation meets opportunity',
           'All cruelty springs from weakess',
...
def generate_records(length_of_name, list_of_domains, total_records, list_of_quotes):
    with open("data.txt", "w") as to_write: # open a data file with write permissions
        for num in range(total_records):
            key = generate_name(length_of_name)+"@"+get_domain(list_of_domains)
            value = get_quotes(quotes)
            to_write.write(key + ":" + value + "\n") # write the email address and the quote to the file
        to_write.write("crshaka@example.com:I'll lead you to a win\n")
        to_write.write("craig@example.com:Stay out of my way or get run over\n")
